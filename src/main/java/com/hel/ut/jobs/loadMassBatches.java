        ...
        try {
        	...
        	try {
            	try {
                    ...
                    messageDetails.setmessageSubject("loadMassBatches job error - " + System.getProperty("server.identity"));
                	...
                	throw new Exception("Error occurred for loadMassBatches  - schedule task",ex);
                } catch (Exception ex1) {
                    Logger.getLogger(loadMassBatches.class.getName()).log(Level.SEVERE, null, ex1);
                }
            	
            	throw new Exception("Error occurredfor loadMassBatches job  - new type schedule task",ex);
            } catch (Exception ex1) {
                Logger.getLogger(loadMassBatches.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
