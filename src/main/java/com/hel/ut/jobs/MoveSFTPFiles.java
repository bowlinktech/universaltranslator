        ...
        try {
        	...
        	try {
            	try {
                    ...
                    messageDetails.setmessageSubject("MoveSFTPFiles job error - " + System.getProperty("server.identity"));
                	...
                	throw new Exception("Error occurred for MoveSFTPFiles  - schedule task",ex);
                } catch (Exception ex1) {
                    Logger.getLogger(MoveSFTPFiles.class.getName()).log(Level.SEVERE, null, ex1);
                }
            	
            	throw new Exception("Error occurred for MoveSFTPFiles job  - new type schedule task",ex);
            } catch (Exception ex1) {
                Logger.getLogger(MoveSFTPFiles.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
