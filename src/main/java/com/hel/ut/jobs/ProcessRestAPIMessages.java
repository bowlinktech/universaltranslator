 ...
 * @author gchan
 */
public class ProcessRestAPIMessages implements Job {
	...
	try {
        	...
        	try {
            	try {
                    ...
                    messageDetails.setmessageSubject("ProcessRestAPIMessages job error - " + System.getProperty("server.identity"));
                	...
                	throw new Exception("Error occurred for ProcessRestAPIMessages  - schedule task",ex);
                } catch (Exception ex1) {
                    Logger.getLogger(ProcessRestAPIMessages.class.getName()).log(Level.SEVERE, null, ex1);
                }
            	
            	throw new Exception("Error occurred for ProcessRestAPIMessages job  - new type schedule task",ex);
            } catch (Exception ex1) {
                Logger.getLogger(ProcessRestAPIMessages.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
