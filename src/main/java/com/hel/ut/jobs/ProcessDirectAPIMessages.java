 ...
 * @author gchan
 */
public class ProcessDirectAPIMessages implements Job {
	...
	try {
        	...
        	try {
            	try {
                    ...
                    messageDetails.setmessageSubject("processDirectAPIMessages job error - " + System.getProperty("server.identity"));
                	...
                	throw new Exception("Error occurred for processDirectAPIMessages  - schedule task",ex);
                } catch (Exception ex1) {
                    Logger.getLogger(ProcessDirectAPIMessages.class.getName()).log(Level.SEVERE, null, ex1);
                }
            	
            	throw new Exception("Error occurred for processDirectAPIMessages job  - new type schedule task",ex);
            } catch (Exception ex1) {
                Logger.getLogger(ProcessDirectAPIMessages.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
