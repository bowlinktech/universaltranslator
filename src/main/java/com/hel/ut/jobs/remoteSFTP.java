 ...
 * @author cmccue
 */
public class remoteSFTP implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        try {
            ...
            transactionInManager.checkRemoteSFTPConfigurations();
        } catch (Exception ex) {
        	try {
		    try {
			/* Send Email to admin*/
			...
			messageDetails.setmessageSubject("remoteSFTP job error - " + System.getProperty("server.identity"));
			...
			throw new Exception("Error occurred for remoteSFTP  - schedule task",ex);
		    } catch (Exception ex1) {
			Logger.getLogger(remoteSFTP.class.getName()).log(Level.SEVERE, null, ex1);
		}
            	throw new Exception("Error occurred for remoteSFTP job  - new type schedule task",ex);
            } catch (Exception ex1) {
                Logger.getLogger(remoteSFTP.class.getName()).log(Level.SEVERE, null, ex1);
            }
        }
