...
<aside class="secondary">
    <nav class="secondary-nav" role="navigation">
	   ...
	   <li role="menuitem" ${param['page'] == 'specs' ? 'class="active"' : ''} ${id > 0 and configurationDetails.stepsCompleted >= 2 ? '' : 'class="disabled"'}><a href="${param['page'] != 'specs' and id > 0 and configurationDetails.stepsCompleted >= 2 ? 'messagespecs' : 'javascript:void(0);'}" title="Detail Message Specs">Message Specs</a></li>
	   <c:choose>
	       <c:when test="${configurationDetails.type == 1}">
		    <li role="menuitem" ${param['page'] == 'mappings' ? 'class="active"' : ''} ${id > 0 and configurationDetails.stepsCompleted >= 3 and mappings == 1 ? '' : 'class="disabled"'}>
			<a href="${param['page'] != 'mappings' and id > 0 and configurationDetails.stepsCompleted >= 3 and mappings == 1 ? 'mappings' : 'javascript:void(0);'}" title="Field Settings">Field Settings</a>
		    </li>
	       </c:when>
	       <c:otherwise>
		   <li role="menuitem" ${param['page'] == 'mappings' ? 'class="active"' : ''} ${id > 0 and configurationDetails.stepsCompleted >= 3 and mappings == 1 ? '' : 'class="disabled"'}>
		       <a href="${param['page'] != 'mappings' and id > 0 and configurationDetails.stepsCompleted >= 3 and mappings == 1 ? 'mappings' : 'javascript:void(0);'}" title="Field Mappings">Field Mappings</a>
		   </li>
	        </c:otherwise>
	   </c:choose>
	   <li role="menuitem" ${param['page'] == 'translations' ? 'class="active"' : ''} ${id > 0 and configurationDetails.stepsCompleted >= 4 ? '' : 'class="disabled"'}><a href="${param['page'] != 'translations' and configurationDetails.stepsCompleted >= 4 ? 'translations' : 'javascript:void(0);'}" title="Data Translations">Data Translations</a></li>
